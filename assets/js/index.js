...
document.getElementById("btnRegistrar").addEventListener("click", ()=>{
    ...
    let imageSrc = imageSrcBg.slice(5, imageSrcBg.length -2);
    let ki = document.getElementById("poderPelea");

    let nuevoParticipante;

    if(raza.value == "Saiyajin"){
        nuevoParticipante = new Saiyajin(nombre.value, imageSrc, ki.value, raza.value);
    }else if (raza.value == "Humano"){
        nuevoParticipante = new Humano(nombre.value, imageSrc, ki.value, raza.value);
    }

    if(raza.value && ki.value && nombre.value && imageSrcBg){
        participantes.push(nuevoParticipante);
        nombre.selectedIndex = 0;
        raza.selectedIndex = 0;
        previewElement.style.backgroundImage="none";
        imageSrcBg = previewElement.style.backgroundColor="#f0f0f0f0";
        ki.value = "";
        reloadTable();
    }else{
        alert("Falta datos por llenar")
    }
    //participantes.push(nuevoParticipante);
});
    ...
    const reloadTable = () =>{
        ...
        participantes.forEach( (p,i) =>{
            ...
            <div class="card-body">
                ...
                <h6 class="card-text">Poder de Pelea: <span> ${p.getPoder()}</span></h6>
